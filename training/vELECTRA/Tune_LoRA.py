"""
Tune_LoRA.py
Fine-tune FPTAI/velectra-base-discriminator-cased using LoRA (Low-Rank Adaptation).

Output (per run) written to result/results_vELECTRA/lora_base/:
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune FPTAI/velectra-base-discriminator-cased with LoRA.")
    ...
    output_dir = "result/results_vELECTRA/lora_base"
    ...
    # ── Model + LoRA ───────────────────────────────────────────────────────────
    print(f">>> Initializing FPTAI/velectra-base-discriminator-cased with LoRA...")
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)

    lora_config = LoraConfig(
        r=args.lora_rank,
        lora_alpha=args.lora_alpha,
        target_modules=["query", "key", "value"] if "electra" in ["roberta", "bert", "electra"] else ["query_proj", "key_proj", "value_proj"],
