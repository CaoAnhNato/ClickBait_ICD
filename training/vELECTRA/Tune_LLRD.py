"""
Tune_LLRD.py
Fine-tune FPTAI/velectra-base-discriminator-cased for Vietnamese Clickbait Detection using Layer-wise Learning Rate Decay.

Output (per run) written to result/results_vELECTRA/llrd_base/:
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune FPTAI/velectra-base-discriminator-cased using LLRD."
    )
    ...
    output_dir = "result/results_vELECTRA/llrd_base"
    ...
    if args.freeze_layers > 0:
        ...
        for name, param in model.named_parameters():
            if name.startswith("electra.embeddings."):
                param.requires_grad = False
            elif name.startswith("electra.encoder.layer."):
