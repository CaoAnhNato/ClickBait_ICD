"""
Tune_LoRA.py
Fine-tune xlm-roberta-base using LoRA (Low-Rank Adaptation).

Output (per run) written to result/results_XLMR/lora_base/:
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune xlm-roberta-base with LoRA.")
    ...
    output_dir = "result/results_XLMR/lora_base"
    ...
    # ── Model + LoRA ───────────────────────────────────────────────────────────
    print(f">>> Initializing xlm-roberta-base with LoRA...")
    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)

    lora_config = LoraConfig(
        r=args.lora_rank,
        lora_alpha=args.lora_alpha,
        target_modules=["query", "key", "value"] if "roberta" in ["roberta", "bert", "electra"] else ["query_proj", "key_proj", "value_proj"],
