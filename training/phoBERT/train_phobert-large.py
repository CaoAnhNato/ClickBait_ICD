"""
train_phobert-large.py
Fine-tune PhoBERT-large for Vietnamese Clickbait Detection.

Pipeline mirrors train_phobert.py (base-v2) with the following differences:
  - Model   : vinai/phobert-large  (24-layer, 1024-hidden)
  - Defaults: smaller batch, lower LR, more GA steps (GPU memory)
  - Weights : uploaded to WandB as artifact (not saved locally) to support
              training on cloud VMs. Pass --no-wandb to save locally instead.

Output (per run) written to result/results_phoBERT/phobert_large/:
  ├── best_model/                  # Saved only when --no-wandb is set
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune PhoBERT-large for Clickbait Detection."
    )
    parser.add_argument("-e",  "--epochs",       type=int,   default=10,   help="Max training epochs.")
    parser.add_argument("-b",  "--batch-size",   type=int,   default=4,    help="Per-device batch size.")
    parser.add_argument("-ga", "--gradient-accumulation", type=int, default=8,
                        help="Gradient accumulation steps (effective batch = b × ga).")
    ...
    parser.add_argument("-fl", "--freeze-layers", type=int,  default=0,
                        help="Number of encoder layers to freeze from the bottom (0-24).")
    parser.add_argument("-p",  "--patience",     type=int,   default=3,    help="Early-stopping patience.")
    parser.add_argument("-n",  "--run-name",     type=str,   default=None, help="WandB run name.")
    parser.add_argument("--no-wandb", action="store_true",
                        help="Disable WandB; save best model locally instead.")
    ...
    output_dir = "result/results_phoBERT/phobert_large"
    ...
    if use_wandb:
        wandb.init(
            ...
            project="ICD_Model",
            name=args.run_name or "phobert-large",
            config={
    ...
    if new_f1 > old_f1:
        ...
        # 2. Save config (hyperparams + final metrics) ------------------------------
        final_metrics = {
        ...
        # 3. Save / upload best model weights --------------------------------------
        if use_wandb and wandb.run is not None:
            # Upload model as WandB artifact (cloud VM workflow)
            print(">>> Uploading model to WandB as artifact...")
            trainer.save_model(best_model_dir)
            tokenizer.save_pretrained(best_model_dir)

            artifact = wandb.Artifact(
                name=f"phobert-large-{wandb.run.id}",
                type="model",
                description="Best PhoBERT-large checkpoint for clickbait detection",
            )
            artifact.add_dir(best_model_dir)
            wandb.log_artifact(artifact)

            # Also log metrics and report file
            wandb.log({f"test/{k}": v for k, v in final_metrics.items()})
            wandb.save(report_csv_path)
            wandb.save(config_path)
            wandb.finish()
            print(">>> Model artifact and metrics uploaded to WandB.")
        else:
            # Local save only (--no-wandb mode)
            trainer.save_model(best_model_dir)
            tokenizer.save_pretrained(best_model_dir)
            print(f">>> Best model saved locally → {best_model_dir}")
    else:
